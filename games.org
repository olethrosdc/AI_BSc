...
Any system involving interaction between multiple agent can be described through game theory. One question is how to define the *preferences* of each agent.
#+BEAMER: \pause
**** Human preferences
- These are typically *unknown*.
- They might not be *compatible* with a mathematical model.
- We usually *assume* the expected utility hypothesis.
...
To keep things simple, we focus on 2-player games.
#+BEAMER: \pause
**** Player action space $A \times B$.
- Player 1 chooses an action $a \in A$.
- Player 2 chooses an action $b \in B$.

#+BEAMER: \pause
Then, both players choose their actions:
**** Reward functions $\rho^i : A \times B \to \Reals$
- Player 1 obtains reward $\rho^1(a, b)$
- Player 2 obtains reward $\rho^2(a, b)$ .

The type of game we have depends on the order actions and the information structure.
...
- $a = 0$ is a *strictly dominant* action. Given any $b$, it is strictly better to play $a = 0$, i.e. $\rho^1(0, b) > \rho^1(1, b)$.
...
\begin{tikzpicture}[level 1/.style={sibling distance=8em},
   ...
   child {node[select] {$a$}
     child {node[RV] {4,0} edge from parent node[left] {0}}	
     child {node[RV] {3,1} edge from parent node[right] {1}}
     edge from parent
     node[right] {1}
   };    	    
...
*** Normal-form simultaneous-move co-operative games
...
- Both players $a$ receive $\rho(a,b)$.

**** No-coordination: Mixed strategies
...
- Player $i$ receives a reward $\rho^i(\ba)$
**** Mixed strategies
Players *independently* draw actions $a^i$ from *strategies* $\pol^i(a^i)$, i.e.
\[
\pol(\ba) = \prod_i \pol^i(a^i).
\]
(This means they cannot coordinate).

The *expected utility* of the strategy $\pol$ is then
\[
\E[\rho^i \mid \pol] = \sum_{\ba} \pol(\ba) \rho^i(\ba).
\]
...
- $n$ players
- Actions $\ba = (a^1, \ldots, a^n)$.
- Payoffs $\rho_i(a)$

**** Pure Nash equilibrium
A joint action $\ba$ is a *pure Nash* equlibrium if no player $i$ can play some alternative action $\hat{a}^i$ and obtain a higher payoff:
\[
\rho^i(\hat{a}^i, a^{-i}) \leq \rho^i(\ba) \forall i, \hat{a}^i,
\]
where $a^{-i} = (a_1, \ldots, a^{i-1}, a^{i+1}, \ldots, a_n)$ are the actions of all players apart from $i$.

**** Nash equilibrium
A strategy profile $\pol = (\pol^1, \ldots, \pol^n)$, is a *Nash* equlibrium if no player $i$ can deviate to obtain a higher payoff:
\[
\E[\rho^i \mid \hat{a}^i, \pol^{-i}] \leq \E[\rho^i \mid \pol] 
\]
...
&= \rho^i(s) + \sum_{a \in A} \pol_{I(s)}(a \mid s) \sum_{j} V^{i,\pi}_{t+1}(j) P(j\mid s, a)
\end{align}
