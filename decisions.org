...
- The action determines the *outcome* distribution $P(\omega \mid a)$
- The utility may depend on *both* the outcome and reward $U(\omega, a)$.
- The *expected utility* for an action is
\[
...
#+BEAMER: \pause
**** Expected utility of action $a$: $\E_P[U |a] = \sum_{\omega \in \Omega} U(\omega, a) P(\omega \mid a)$.
#+BEGIN_SRC python
  # U: A matrix U[a, w]
  # P: A matrix P[w, a]
  # a: The action taken
...
**** The expected utility hypothesis                           :B_alertblock:
:PROPERTIES:
...
\]
*** The gradient method: multiple dimensions
