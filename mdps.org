...
*** Simple betting game strategies
**** Always play
\[
...
*** The utility
**** The utility
This is simply the sum of rewards we obtain until the game ends.
\[
U_t = \sum_{k=t}^T r_k
\]
**** The expected utility of a policy
Since rewards are random, we want to maximise expected utility
\[
\E^\pol_\mdp[U_t] = \sum_{k=t}^T \E^\pol_\mdp[r_k]
\]
**** The optimal policy for a given MDP
\[
\pol^*(\mdp) = \argmax_\pol \E^\pol_\mdp[U_t]
\]
...
\[
V^\pol_{\mdp,t}(s) \defn \E^\pol_\mdp(U_t \mid s_t = s)
\]
#+BEAMER: \pause
**** The state-action value function
This is the expected utility obtained by following a policy $\pol$ starting from some state $s$ and *playing action $a$*
\[
Q^\pol_{\mdp, t}(s, a) \defn \E^\pol_\mdp(U_t \mid s_t = s, a_t = a)
\]
#+BEAMER: \pause
**** The optimal value function
There is some policy $\pol^*$ satisfying
\begin{align*}
V_{\mdp,t}^*(s) &\defn V_{\mdp,t}^{\pol^*}(s) \geq V_{\mdp,t}^\pol(s) \qquad \forall \pol, s
\\
Q_{\mdp,t}^*(s, a) &\defn Q_{\mdp,t}^{\pol^*}(s, a) \geq Q_{\mdp,t}^\pol(s, a) \qquad \forall \pol, s, a
\end{align*}
...
\uncover<3->{
&= \E^\pol\left[\sum_{k=t}^T r_k \middle| s_t\right]\\
}
\uncover<4->{
&= \E^\pol[r_t \mid s_t] + \E^\pol\left[\sum_{k=t+1}^T r_k \middle| s_t  \right]\\
}
\uncover<5->{
&= \E^\pol[r_t \mid s_t] + \E^\pol\left[U_{t+1}| s_t \right]\\
}
\uncover<6->{
&= \E^\pol[r_t \mid s_t] + \sum_{s_{t+1} \in S} \Pr^\pol(s_{t+1} \mid s_t) \E^\pol\left[U_{t+1}| s_{t+1} \right]\\
}
\uncover<7->{
&= \E^\pol[r_t \mid s_t] +  \sum_a \pol(a \mid s_t) \sum_{s_{t+1} \in S} P_\mdp(s_{t+1} \mid s_t, a) V^\pol_{t+1}(s_{t+1}).
}
\end{align*}
#+BEAMER: \pause
#+BEAMER: \pause
