...
**** Generic depth-first search
  \begin{algorithmic}
    ...
    \STATE $F = F \setminus \{n\}$ : mark $n$ as searched
	\FOR {$j \in F: \langle n,j \rangle \in A$}
	\STATE $\texttt{DepthFirst}(V, j)$
	\ENDFOR
  \end{algorithmic}
**** Discussion
...
- At the root, we have the value 'true' for the child reaching the goal.
- The path is not saved, however.
**** Goal-DFS
#+BEGIN_SRC python
  from include/graph import graph
  searched = set()
  def GoalDFS(graph, current):
