#+TITLE: Shortest Path Problems and Informed search
...
\begin{algorithmic}
\STATE \textbf{Dijkstra($Y, x$)} \# $Y$ are goal states $Y$, $x$ is starting state.
\STATE $C(s) = 0$ for $s \in Y$, $C(s) = \infty$ for $s \notin Y$, $F_0 = Y$
\FOR {$t = 0, 1, \ldots$}
...
- For any other $s$, we will show that if $s'$ is a next state and
  $C(s') \leq C^*(s')$ then $C(s) \leq C^*(s)$.
\begin{align*}
\uncover<3->{
C(s)
&=
...
#+BEAMER: \pause
#+BEAMER: \pause
For the optimal policy $\pi^*$, $C^{\pi^*}(s) = C^*(s) \leq C^\pi(s)$ for any $\pi$. Finally,
#+BEAMER: \pause
\[
...
**** Finding a shortest path from $X$ to $Y$.
#+ATTR_BEAMER: :overlay <+->
- Stops when a *starting* state is found.
...
**** Finding all optimal paths to $Y$.
#+ATTR_BEAMER: :overlay <+->
...
- We denote the total cost of some path $s_1, \ldots, s_t$ as: $$C(s_1, \ldots, s_t)$$
- The remaining cost from $s_t$ to the goal using some policy $\pi$ as $$C^\pi(s_t)$$
...
\STATE \textbf{function} \texttt{Heuristic Search}($G, x, f$)
...
**** Proof
#+ATTR_BEAMER: :overlay <+->
- *Existence*. There is a finite number of paths that will be explored, as (a) the longest possible path to a goal is $C^*(0)/\epsilon$ and (b) the branching factor is bounded.
- *Optimality*. The proof is by contradiction. Let as assume that $A^*$ finds some $\pi \neq \pi^*$ so that $C(\pi) > C(\pi^*)$ $\Rightarrow$ there is a node $n$ on the path where $a^*$ is optimal policy, but we instead follow $\pi$ to $s_1, s_2, \ldots$. However, since $C(\pi) > C(\pi^*)$ there must be some $t$ such that $C(n, s_1, \ldots, s_t) > C^{\pi^*}(n)$. But then, to expand $\pi$ requires that $C(n, s_1, \ldots, s_t) + h(s') < h(s) \leq C^{\pi^*}(n)$.
...
- For $k \in 1, \ldots, |S| - 1$: \[ \forall s \in S: C_{k}(s) = \min_a c(s,a) + C_{k-1}(\tau(s,a)) \]
...
*** Bellman-Ford Algorithm
Here is a full pseudo-code for the Bellman-Ford algorithm
\begin{algorithmic}
