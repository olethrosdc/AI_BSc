...
*** Constraint Satisfaction Problems (CSP) with *binary* constraints
**** Variables
- A set of variables $\{x_1, \ldots, x_n\} \in X$
- Each variable can take values in $x_i \in X_i$ (its *domain*)
#+BEAMER: \pause
**** Binary constraints
- $c_{i,j} : X_i \times X_j \to \{0, 1\}$.
- A constraint $c_{ij}(x_i, x_j)$ is *violated* when it has the value $1$.
#+BEAMER: \pause
...
*** CSP with general constraints
**** Example: Meeting scheduling                             :B_exampleblock:
:PROPERTIES:
...
**** General constraints
- Each constraint $c_i$ is linked to a set $S_i \subset X$ of variables.
- This can be represented graphically with links from the constraint to those variables.
**** Graph representation
\begin{tikzpicture}[every edge quotes/.style = {auto, font=\footnotesize, sloped}]
      \node[RV] at (0,0) (1) {$x_1$};
      \node[RV] at (0,2) (2) {$x_2$};
...
*** Constrained optimisation problems (COP)
**** Variables
- A set of variables $\{x_1, \ldots, x_n\} \in X$
- Each variable can take values in $x \in X_i$, with $X \in \prod_i X_i$.
**** General constraints
- $c_{i} : X \to \{0, 1\}$, with $S_i \subset X$, $i \in [m]$.
**** Objective 
- Maximise $u: X \to \Reals$,
- s.t. (such that) $c_i(X) = 0$ for all $i \in [m]$.
**** Special cases:
- Linear utility over nodes: $u(X) = \sum_i u_i(x_i)$
- Linear utility over edges: $u(X) = \sum_{ij} u_{ij}(x_i, x_j)$
...
\[|x| = \sum_{(s,i) \in E} x_{si}  = \sum_{(j,t) \in E} x_{jt}\]
**** Flow constraints
The flow satisfies the following constraints:
- Capacity constraint: $x_{ij} \leq c_{ij}$
- Conservation of flows:
\[
\forall n \in N \setminus \{s, t\}  
\sum_{i : (i,j) \in E, x_{ij} > 0} x_{ij}
=
\sum_{j : (i,j) \in E, x_{ji} > 0} x_{ji}.
\]
**** The maximum network flow problem
Maximise $|x|$ while satisfying the capacity and conservation constraints.

** Algorithms
*** Naive search
- Set all variables to unassigned
- Go linearly over the variables
- Set the current variable to a value that does not violate constraints linked to its predecessors, and then search the next variable.
- If this is not possible, go to the previous variable again.
- The algorithm can be seen as a tree search (to be done on the board).
- The algorithm is guaranteed to complete given sufficient time.
  
*** Local search
- Choose a variable arbitrarily.
- Select a value for the variable that reduces the number of constraint violations
- This algorithm may get stuck.
  
**** Randomised local search
- The randomised version of the algorithm chooses a value for each variable with decreasing probability for the number of constraint violations.
- The algorithm has a positive probability of completion.
...
- Any atom $A$ corresponds to a subset of $\Omega$.
