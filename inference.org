...
- The negative event $\neg A = \Omega \setminus A$ is the set $\{\omega \in \Omega \wedge \omega \notin A$.
...
**** Conditional probability :B_definition:
    :PROPERTIES:
...
- $P_\param$ is sometimes called the *likelihood*
  
*** Independence
**** Independence                                              :B_definition:
:PROPERTIES:
:BEAMER_env: definition
:END:
...
This means that knowing $B$ does not tell us anything about $A$ and vice-versa.
...
- $P(A | B, C) = P(A | C)$
- or equivalently $P(A \cap B \mid C) = P(A | C) P(B | C)$.
This means that knowing $B$ does not tell us anything about $A$ and vice-versa.



*** Conditional probability game
You are each given a pair of cards, either black (\spadesuit, \clubsuit) or red (*\vardiamondsuit*, *\varheartsuit*).
#+BEAMER: \pause
Keep the cards face down for now. 
|--------+-------+-------|
| Card 1 | Car 2 | Prob. |
|--------+-------+-------|
| *R*    | *R*   |  1/3  |
...
|--------+-------+-------|
#+BEAMER: \pause
If everybody opens *one* card, what is the proportion of cards that are going to be *Red*?
#+BEAMER: \pause
|------+--------+-----+--------------------|
| Open | Closed | Pr. | $Pr(R \mid Open )$ |
|------+--------+-----+--------------------|
| *R*  | *R*    | 2/6 | $2/6 \times 3/1    |
| *R*  | B      | 1/3 |                    |
| B    | *R*    | 1/3 |                    |
| B    | B      | 2/6 |                    |
|------+--------+-----+--------------------|
...
#+BEAMER: \pause
**** Calculation from conditional probability
P(R_2|R_1) = P(R_2 \cap R_1) / P(R_2)
...
\begin{align*}
\alert<2>{P(B | A)} &\alert<2>{= \frac{P(A \cap B)}{P(A)}}
\Rightarrow
\alert<3,5>{P(A \cap B)} = \alert<4,6>{P(B | A) P(A)}\\
P(A | B) &= \frac{\alert<3>{P(A \cap B)}}{P(B)}
\Rightarrow
 P(A | B) =  \frac{\alert<4>{P(B | A) P(A)}}{P(B)}
\end{align*}
By marginalisation:
\[
P(B) = \alert<5>{P(B \cap A)} + P(B \cap \neg A)
 = \alert<6>{P(B | A) P(A)} + P(B | \neg A) P(\neg A).
\]
\end{proof}



*** Bayes's theorem: the general case
...
- Parameter: $\param \in \Param$.
- Prior: $\bel(\param)$.
- Data: $x$, sampled from  $P_{\theta^*}$.
- Likelihood: $\{P_\param(x) : \param \in \Param\}$ (in model family)
- Posterior:
  \[
  \bel(\param \mid x) = \frac{P_\param(x) \bel(x)}{\sum_{\param' \in \Param} P_{\param'}(x) \bel(\param')}
  \]
...
#+BEAMER: \pause
**** Uniform distribution                                      :B_definition:
:PROPERTIES:
:BEAMER_env: definition
:END:
$P$ is a uniform distribution on $\Omega$ $\Leftrightarrow$ $P(A)= |A|/|\Omega|$  $\forall A \subset \Omega$.\footnote{Formally, for $A$ in a $\sigma$-algebra.}
**** Bernoulli distribution                                  :B_exampleblock:
:PROPERTIES:
:BEAMER_env: exampleblock
:END:
If $f(\omega) = \begin{cases} 1, &\omega \leq \theta\\ 0, &\omega \geq \theta\end{cases}$,
with $P$ uniform on $\Omega$, then we say that $f$ is Bernoulli distributed with parameter $\theta$ and write
\[
f \mid \theta \sim \Ber(\theta)
\]
#+BEAMER: \pause
**** Exponential distribution                                :B_exampleblock:
:PROPERTIES:
:BEAMER_env: exampleblock
:END:
If $f(\omega) = $\frac{\ln (\omega)}{\lambda}$ and $\omega$ is uniform
...
#+BEAMER: \pause
**** The density of $f$
When $f$ takes an infinite number of values, it is easier to talk about the *density $p_f$*, which is linked with the probability through integration:
\[
P_f(A) \defn \int_A p_f(x) \, dx.
\]
#+BEAMER: \pause
**** Shorthands for RV
- For RVs $f : \Omega \to \Reals$, we write $P(f \in A)$ to mean $P_f(A)$.
- For RVs $f : \Omega \to X$, where $X$ is a finite set e.g. $\{1, 2, \ldots, n\}$, we write $P(f = x) = P_f(\{x\})$ for any $x \in X$, or even $P(f)$.
- For continuous RVs $f : \Omega \to \Reals$, we use densities as  $p_f(x)$.
...
we can say that $k \sim P_x$, with
$P_f(A) = P(\{\omega : f(\omega) \in A\})$.
\[
\E_P(f) = \sum_{\omega \in \Omega} f(\omega) P(\omega) = \sum_{k \in X} k P_f(k).
\]
...
\]
which then denotes the probability mass of each
...
\begin{center}
    \begin{tikzpicture}
      ...
      \draw[->] (x1) to (x3);
\uncover<2->{
      \node[RV,hidden] at (3,0) (w3) {$\omega_3$};
      \node[RV,hidden] at (-1,0) (w1) {$\omega_3$};
      \node[RV,hidden] at (1,2) (w2) {$\omega_2$};
      \draw[->] (w1) to (x1);
      \draw[->] (w2) to (x2);
      \draw[->] (w3) to (x3);
}
...
We can write the joint distribution of any set of RVs $x_1, \ldots, x_n$, as
\[
...
This holds more generally for sets of random variables.
...
#+BEAMER: \pause
\begin{align*}
P(x_1, x_2, T) &= P(x_1, x_2 | T) P(T)\\
               & = P(x_1 | T) P(x_2 | T) P(T)\\
P(x_1, x_2 | T)&=  P(x_1 | T) P(x_2 | T) P(T)\\
P(x_1, x_2) &= \sum_{i=0}^1 P(x_1 | T = i) P(x_2 | T=i) P(T=i)
\end{align*}
