...
**** Cards
You are each given a pair of cards. Each card is either black (\spadesuit, \clubsuit) or red (*\vardiamondsuit*, *\varheartsuit*).

#+BEAMER: \pause
In particular, you each get two hidden cards, and each combination has a given probability. Keep the cards face down for now. [FILL]
