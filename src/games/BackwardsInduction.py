...
def backwards_induction(game, policy, T):
    ## This is the expected value for states s at time t: V[t,s]
    V = np.zeros([T, game.n_states])
    ## This is the value for state-actions s,a at time t: Q[t, s, a]
    Q = np.zeros([T, game.n_states, game.n_actions])
    ## This is the policy (action taken) at time t, state s: policy[t, s]
    policy = np.zeros([T, game.n_states])

    ## First fill in the value at time T:
    ## (this is *backwards* induction)
    ## The reward is always given by the state.
    ## We assume these rewards and values are all for the maximising player
    ...
    for t in range(T-2, 0, -1):
        ## Here we use a 'sign' variable so that one player is minimising
        if (t % 2)==0:
            sign = 1
        else:
            sign = -1
