...
\only<5->{
	\node[RV,hidden] at (1,0) (1) {1};
}
	\draw[->] (0) to (1);
	}
	\uncover<3->{
	\node[RV, hidden] at (2,0) (5) {5};
	\draw[->] (1) to (5);
	}		
	\uncover<4->{
	\node[RV] at (2,1) (4) {4};
...
\IF {$n \notin S'$}
\STATE $B = \{i \in \child(n) : i \notin \anc(n)\}$. Get the set of unsearched children of $n$.
